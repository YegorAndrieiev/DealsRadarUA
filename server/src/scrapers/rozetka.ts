...
function cleanTitle(title: string): string {
  ...
  return title
    .replace(/[\u200B-\u200D\uFEFF]/g, '')
    .replace(
      /[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{2600}-\u{27BF}\u{1F900}-\u{1F9FF}\u{1F1E0}-\u{1F1FF}]/gu,
      '',
    )
...
function extractDataFromHtml(html: string): Product[] {
  try {
    ...
    $('rz-catalog-tile').each((index, element) => {
      ...
      let priceElement = card.find(
        '[data-testid="red-price-discount"], .red-price',
      );
      if (priceElement.length === 0 || !priceElement.text().trim()) {
        priceElement = card.find('.price');
      }
      if (priceElement.length === 0 || !priceElement.text().trim()) {
        priceElement = card.find(
          '.tile-price__value, .goods-tile__price-value',
        );
      }
      const priceText = priceElement.text().trim();
      const priceTextClean = priceText
        .replace(/[\s\u00A0]/g, '')
        .replace(',', '.');
      ...
      const isUsed =
        cardText.includes('б/в') ||
        cardText.includes('б/у') ||
        cardText.includes('вжива');
